...
<style>
    ...
    /* Member Info Card */
    .member-info-card {
        background: linear-gradient(135deg, rgba(179, 217, 255, 0.15) 0%, rgba(217, 179, 255, 0.1) 100%);
        border: 1px solid rgba(179, 217, 255, 0.3);
        border-radius: 16px;
        padding: 18px;
    }
    ...
    .member-info-item {
        ...
        padding: 8px 0;
        border-bottom: 1px solid rgba(179, 217, 255, 0.2);
        font-size: 13px;
    }
    ...
    <div class="page-header">
        <div class="header-content">
            ...
            <div class="header-text">
                ...
                <p>Update {{ $teamMember->name }}'s profile and information</p>
            </div>
        </div>
    </div>

    <form method="POST" action="/admin/team-members/{{ $teamMember->id }}" id="memberForm">
        @csrf
        @method('PUT')
            ...
            <div class="main-column">
                ...
                <div class="form-card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $teamMember->name) }}" required autofocus placeholder="e.g., John Doe" oninput="updateAvatarPreview()">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" id="phone" name="phone" value="{{ old('phone', $teamMember->phone ?? '') }}" placeholder="e.g., +1 234 567 890">
                ...
                <div class="form-card purple">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="role" name="role" value="{{ old('role', $teamMember->role) }}" required placeholder="e.g., Senior Developer">
                        ...
                        <div class="form-group">
                            ...
                            <select id="department" name="department">
                                ...
                                <option value="Engineering" {{ old('department', $teamMember->department ?? '') == 'Engineering' ? 'selected' : '' }}>Engineering</option>
                                <option value="Design" {{ old('department', $teamMember->department ?? '') == 'Design' ? 'selected' : '' }}>Design</option>
                                <option value="Marketing" {{ old('department', $teamMember->department ?? '') == 'Marketing' ? 'selected' : '' }}>Marketing</option>
                                <option value="Sales" {{ old('department', $teamMember->department ?? '') == 'Sales' ? 'selected' : '' }}>Sales</option>
                                <option value="Operations" {{ old('department', $teamMember->department ?? '') == 'Operations' ? 'selected' : '' }}>Operations</option>
                                <option value="HR" {{ old('department', $teamMember->department ?? '') == 'HR' ? 'selected' : '' }}>Human Resources</option>
                                <option value="Finance" {{ old('department', $teamMember->department ?? '') == 'Finance' ? 'selected' : '' }}>Finance</option>
                            </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="bio" name="bio" placeholder="A brief description about the team member...">{{ old('bio', $teamMember->bio ?? '') }}</textarea>
                ...
                <div class="form-card green">
                    ...
                    <input type="hidden" id="status" name="status" value="{{ old('status', $teamMember->status) }}">
                    
                    <div class="status-toggle">
                        <div class="status-option {{ $teamMember->status == 'active' ? 'active' : '' }}" data-status="active" onclick="selectStatus('active')">
                            <div class="status-icon">✅</div>
                            <div class="status-label">Active</div>
                        </div>
                        <div class="status-option inactive {{ $teamMember->status == 'inactive' ? 'active' : '' }}" data-status="inactive" onclick="selectStatus('inactive')">
            ...
            <div class="sidebar-column">
                ...
                <div class="form-card">
                    ...
                    <div class="avatar-section">
                        @php
                            $parts = explode(' ', $teamMember->name);
                            $initials = '';
                            if (count($parts) >= 2) {
                                $initials = strtoupper($parts[0][0] . $parts[1][0]);
                            } else {
                                $initials = strtoupper(substr($teamMember->name, 0, 2));
                            }
                        @endphp
                        <div class="avatar-preview" id="avatarPreview">
                            <span id="avatarInitials">{{ $initials }}</span>
                        </div>
                        <div class="avatar-name" id="previewName">{{ $teamMember->name }}</div>
                    </div>
                </div>

                <!-- Member Info Card -->
                <div class="member-info-card">
                    <h4>ℹ️ Member Info</h4>
                    <div class="member-info-item">
                        <span class="member-info-label">Created</span>
                        <span class="member-info-value">{{ $teamMember->created_at->format('M d, Y') }}</span>
                    </div>
                    <div class="member-info-item">
                        <span class="member-info-label">Last Updated</span>
                        <span class="member-info-value">{{ $teamMember->updated_at->format('M d, Y') }}</span>
                    </div>
                    <div class="member-info-item">
                        <span class="member-info-label">Member ID</span>
                        <span class="member-info-value">#{{ $teamMember->id }}</span>
                    </div>
                </div>
            </div>
        </div>
